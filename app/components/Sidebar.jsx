...
export default function Sidebar({ isCollapsed, onToggle, onCreateNewCourse }) {
  ...
  return (
    <>
      ...
      {/* Expanded Sidebar */}
      <aside className={`bg-white border-r border-gray-200 min-h-screen transition-all duration-300 shadow-xl ${
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900 font-khula">Course<span className="text-orange-500">Maker</span></h2>
                <p className="text-xs text-gray-500 font-khula">Pro</p>
              </div>
            </div>
            <button
              onClick={onToggle}
              className="w-8 h-8 bg-gray-100 text-gray-600 rounded-lg flex items-center justify-center hover:bg-gray-200 hover:text-gray-800 transition-all duration-300"
            >
          ...
          <nav className="space-y-1">
            {menuItems.map((item) => (
              <a
                key={item.id}
                href={item.href}
                className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all font-khula ${
                  activeItem === item.id
...
<br></br>
          <div className="mt-8 p-4 bg-gradient-to-r from-orange-50 to-orange-100 rounded-lg border border-orange-200 shadow-md">
            <a 
              href="https://ebon-adventure-website.netlify.app/" 
              target="_blank" 
              rel="noopener noreferrer"
              className="block w-full bg-gradient-to-r from-orange-500 to-orange-600 text-white py-2 rounded-lg text-sm font-semibold font-khula hover:from-orange-600 hover:to-orange-700 transition-all duration-300 shadow-md text-center"
            >
              My Game
            </a>
          </div>
